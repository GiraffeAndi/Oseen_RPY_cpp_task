...
vtf_path = os.path.join(res_path, f"trajectory_dipol_no_noise{sep}.vtf")
json_path = os.path.join(res_path, f"trajectory_dipol_no_noise{sep}.json")
...
for i in range(500):
    ...
    angle = 0.5 * i
    H_field = np.array([10.0 * np.cos(angle), 10.0 * np.sin(angle), 0.0])
