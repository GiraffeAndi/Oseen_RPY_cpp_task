...
vtf_path = os.path.join(res_path, f"trajectory_tri_inwards{sep}.vtf")
#json_path = os.path.join(res_path, f"trajectory_tri_inwards{sep}.json")
...
part1 = system.part.add(pos = center + [0.0, 0.0, 0.0], rotation=[True, True, True], type = 0, dip=[-np.sqrt(3)/2, 0.5, 0.0])
part2 = system.part.add(pos = center + [sep, 0.0, 0.0], rotation=[True, True, True], type = 0, dip=[np.sqrt(3)/2, 0.5, 0.0])
part3 = system.part.add(pos = center + [sep/2, (np.sqrt(3)/2) * sep, 0], rotation=[True, True, True], type = 0, ext_force=[0.0, -6.0*np.pi, 0.0], dip=[0.0, 1.0, 0.0])

"""Setup for Brownian mechanics"""
 
system.thermostat.set_brownian(kT = 0.0, gamma = 1.0, gamma_rotation = 1.0, seed = 1)
